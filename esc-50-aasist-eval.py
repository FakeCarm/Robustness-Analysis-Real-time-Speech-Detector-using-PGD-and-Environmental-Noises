...
class ESC_50(Dataset):
    def __init__(self, path_to_database, path_to_protocol):
        ...
        self._path_to_audio = os.path.join(self._path_to_database, "eval")
        self._formato = '.wav'
        ...
        protocol = os.path.join(self._path_to_protocol, 'esc50' + '.' + 'csv')
        ...
        for line in lines:
            ...
            if len(parts) == 7:
                ...
                if os.path.exists(filepath):
                    ...
                    self.all_info.append((filename, 0))   
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return waveform, filename, y
        except Exception as e:
            print(f"Error loading {filepath}: {e}")
            return torch.zeros(1, 16000), filename, y
...
def generate_audio(model_path, use_fp16, dataset_path, output_dir, model_type):
    
    os.makedirs(output_dir, exist_ok=True)
    
    
    file_path = os.path.join(output_dir, f'sample_probability_ESC-50.txt')
    # 2. Controlla se NON esiste
    if os.path.exists(file_path):
        exit("File prob già esistente in ", file_path)
    ...
    dataset = ESC_50(dataset_path, dataset_path)
    ...
    noisy_audio, sr = load_16k_mono(path_noisy_audio)
    ...
    with open(os.path.join(output_dir, 'sample_probability_ESC-50.txt'), 'w') as f:
        for filename, pred in zip(filenames_list, predictions_adv):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='ESC-50 EVAL AASIST LIGHT')
    ...
    dataset_path = "../ESC-50"
    output_dir = f"./esc-50_probabilities/{model_type}"
    use_fp16 = True
    generate_audio(model_path, use_fp16, dataset_path, output_dir, model_type)