...
from dataset_features import ASVspoof5
...
def evaluate_model(model_path, dataset_path, output_dir, model_type):
    ...
    try:
        ...
        ADD_MODEL.eval()
        #summary(ADD_MODEL, input_size=(1, 1024, 49)) 
        #return 1
    ...
    dataset_path = os.path.join(dataset_path, model_type)
    test_dataset = ASVspoof5(dataset_path, 'eval', 'xls')
    print(f"Found {len(test_dataset)} samples in test dataset.")
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False, num_workers=4)

    if len(test_dataset) == 0:
        raise RuntimeError(
            f"Nessun file .pt trovato. Controlla path: {os.path.abspath(dataset_path)} "
            f"e la cartella eval/xls-r-5"
        )
    ...
    predictions = (all_scores_by_fake >= genuine_threshold).astype(int)
...
if __name__ == "__main__":
    ...
    results = evaluate_model(args.model, args.data, args.output_dir, args.model_type)
    if results == 1:
