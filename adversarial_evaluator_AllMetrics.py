...
def evaluate(path_og_sample_prob,path_ambient_sample_prob, output_dir, iteraz, ep):
    ...
    with open(path_og_sample_prob,"r") as f:
        ...
        for line in f.readlines():
            filename_extended ,scores , label, predicted = line.strip().split(" ")
            if model_type == "light":
                pre, name, attack, id_attack, l = filename_extended.split("_")
            else:
                pre, name, l = filename_extended.split("_")
            filename = pre + "_" + name
            dic_filename_og[filename] = (label,scores,predicted)
    ...
    for sample in list_filenames_og:
        ...
        label = int(label)
        if label == 1:
            if sample not in dic_filename_adv:
                missed += 1
                continue
        ...
        '''if predicted_og != 9999:
            if predicted != label:
                if label == 1: # Se il campione originale è bonafide
                    if predicted_og != predicted: # Se il modello classifica correttamente il campione originale ma non l'adversarial, cio significa che l'adversarial ha avuto successo nel far cambiare la predizione del modello, quindi è una misclassificazione "corretta" da parte dell'attacco.
    ...
    results = {
                'Eps':  ep,
                'Iter':  iteraz,
                'Threshold Modello Originale': 0.7,
                'EER (%)': eer * 100,
                'min_DCF': minDCF,
                ...
                'False Positive': fp,
                'False Negative': fn,
                'Missed adv samples': missed
        }

    results = make_json_serializable(results)

    # Salva risultati in file
    with open(os.path.join(output_dir, f'evaluation_adversarial_Iter{iteraz}_Eps{ep}.json'), "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--trasf', type=bool, required=True, help='True for evaluate Trasferability else False')
    args = parser.parse_args()
    model_type = args.model_type
    iteraz = args.iter
    ep = args.eps
    trasf = args.trasf
    path_og_sample_prob = "evaluation_results/sample_probability.txt"

    if trasf == True:
        print("ANALISI DI TRASFERIBILITA")
        path_adversarial_prob = f"adversarial_results_RAWNET/sample_probability_adversarial_Iter{iteraz}_Eps{ep}.txt"
        output_dir = f"evaluation_results_adversarial_Rawnet"
    else:
        print("ANALISI DI ROBUSTEZZA")
        path_adversarial_prob = f"adversarial_results/sample_probability_adversarial_MaxIter{iteraz}_EpsStep{ep}.txt"
        output_dir = f"evaluation_results_adversarial_robustness"
    # Per valutare il light perchè al momento le cartelle hanno un nome diverso.
    #path_ambient_sample_prob =f"ambient_noise_results/{nome_amb_audio}/sample_probability_ambient_adversarial_SNR{snr}.txt"
    
    
    evaluate(path_og_sample_prob, path_adversarial_prob, output_dir, iteraz, ep)
