...
def generate_audio(model_path, use_fp16, dataset_path, output_dir="./adversarial_results/", adversarial_sample_path="./adversarial_samples/"):
    ...
    try:
        ...
        # aggiorno anche la config per coeren 
    ...
    scores_by_iter_step = [[[] for _ in range(len(eps))]
                       for _ in range(len(max_iters))]
    ...
    for n_iter, max_iter in enumerate(max_iters):
        ...
        for n_eps, ep in enumerate(eps):
            ...
            scores = np.array(scores_by_iter_step[n_iter][n_eps])
            ...
            with open(os.path.join(output_dir, f'sample_probability_adversarial_MaxIters{max_iters[n_iter]}_EpsStep{eps[n_eps]}.txt'), 'w') as f:
